...
const fragmentShader = `
  ...
  void main() {
      ...
      if(uv.y < 1.) tex.g = texture2D(uTexture, texScale).g;
...
const GalleryScroll: React.FC<GalleryScrollProps> = ({ images }) => {
  ...
  useLenis(({ velocity }) => {
    ...
    glImagesStripsRef.current.forEach((strip, index) => {
      const scrollDirection = index % 2 === 0 ? -1 : 1
      strip.scrollY += velocity * scrollDirection * strip.scrollSpeedOffset
      const direction = Math.sign(velocity)

      strip.images.forEach((glImage) => {
        glImage.update(strip.scrollY, velocity, direction, containerRef.current!.clientHeight)
      })
    })
  })
